...
function showNotification(message, isError = false) {
    ...
    const iconHtml = `<i data-lucide="${
        isError ? "x-octagon" : "check-circle"
    }" class="w-6 h-6"></i>`;
...
function renderStepIndicators() {
    ...
    STEPS.forEach((step) => {
        ...
        if (isCompleted)
            circleEl.classList.add("bg-secondary-green", "scale-110");
        else if (isActive)
        ...
        circleEl.innerHTML = isCompleted
        ...
        titleEl.className =
            "text-xs mt-3 w-max text-center absolute left-1/2 transform -translate-x-1/2 transition duration-300 ease-in-out";
        titleEl.style.top = "56px";
        if (isCompleted)
            titleEl.classList.add("text-secondary-green", "font-semibold");
        else if (isActive)
            titleEl.classList.add("text-primary-blue", "font-bold");
...
function updateProgress() {
    ...
    if (currentStep === totalSteps) {
        ...
        if (tingkatAktif === 10)
            document.getElementById("summary-jurusan").textContent =
                document.getElementById("jurusan")?.value || "-";
        document.getElementById("summary-kontak").textContent =
    ...
    if (currentStep < totalSteps) {
        ...
        if (currentStep === 1) {
            ...
            if (nisInput) {
                ...
                try {
                    const res = await fetch(`/check-nisn?nisn=${nisn}`);
                    ...
                    if (data.exists) {
                        ...
                        showNotification(
                            "NISN sudah terdaftar. Silakan gunakan yang lain.",
                            true
                        );
                        return;
                    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    jalurSelect.addEventListener("change", async function () {
        ...
        uploadContainer.innerHTML =
            '<p class="text-gray-500 text-sm">Memuat syarat...</p>';

        if (!jalurId) {
            uploadContainer.innerHTML =
                '<p class="text-gray-500 text-sm">Pilih jalur pendaftaran terlebih dahulu.</p>';
            return;
        }

        try {
            const response = await fetch(`/api/syarat-by-jalur/${jalurId}`);
            const data = await response.json();

            if (!data.length) {
                uploadContainer.innerHTML =
                    '<p class="text-gray-500 text-sm">Tidak ada syarat aktif untuk jalur ini.</p>';
                return;
            }
            ...
            data.forEach((syarat, index) => {
                uploadContainer.innerHTML += `
                    ...
                    syarat.syarat
                        ...
                        <input type="file" name="syarat_file_${
                            syarat.id
            ...
            uploadContainer.innerHTML =
                '<p class="text-red-500 text-sm">Terjadi kesalahan saat memuat syarat.</p>';
        }
    });
});
