...
<section id="daftar-siswa-list" class="py-20 bg-neutral-light">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div id="applicants-table-container" class="mt-8 relative" data-aos="fade-up" data-aos-delay="200" data-tingkat="{{ $tingkatAktif->tingkat ?? 0 }}">
            ...
            <div id="table-content" class="opacity-0 transition-opacity duration-500 ease-in-out"></div>
        </div>
        
        <div id="pagination-controls" class="mt-8 flex justify-center space-x-2 opacity-0 transition-opacity duration-300 ease-in-out"></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let tingkatAktif = parseInt(tableContainer.dataset.tingkat || 0, 10);
    ...
    function fetchAllApplicants() {
        ...
        .then(data => {
            ...
            tingkatAktif = parseInt(data.tingkat || 0, 10); // ambil tingkat dari server
            tableContainer.dataset.tingkat = tingkatAktif;
    ...
    function fetchApplicantsUpdates() {
        ...
        .then(data => {
            if (data.applicants && data.applicants.length !== allApplicants.length) {
                 allApplicants = data.applicants;
                 tingkatAktif = parseInt(data.tingkat || 0, 10);
                 tableContainer.dataset.tingkat = tingkatAktif;
    ...
    function filterApplicants(applicants, searchTerm) {
        ...
        return applicants.filter(applicant => 
            (applicant.nomor_resi && applicant.nomor_resi.toLowerCase().includes(lowerSearchTerm)) ||
    ...
    function renderTableAndPagination(data, page) {
        ...
        setTimeout(() => {
            ...
            if (!paginatedData || paginatedData.length === 0) {
                ...
                `;
                if (tingkatAktif === 10) headerCols += `<th class="px-4 py-3 text-xs font-semibold uppercase tracking-wider">Jurusan Dipilih</th>`;
                ...
                paginatedData.forEach((applicant, index) => {
                    ...
                    rows += `
                        <tr class="${rowClass} hover:bg-yellow-50/50 transition duration-150">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-bold text-primary-blue">${applicant.nomor_resi ?? '-'}</td>
                            ...
                            ${tingkatAktif === 10 ? `<td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-secondary-green">${applicant.jurusan || '-'}</td>` : ''}
                        </tr>
                    `;
                });
            ...
            renderPagination(Math.ceil(data.length / pageSize), currentPage, data);
    ...
    function renderPagination(totalPages, currentPage, data) {
        ...
        if (startPage > 1) {
            ...
            if (startPage > 2) buttons += `<span class="px-2 py-2 text-sm text-gray-500">...</span>`;
        }

        for (let i = startPage; i <= endPage; i++) {
            const activeClass = i === currentPage ? 'bg-primary-blue text-white border-primary-blue shadow-md' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100 hover:border-primary-blue';
            buttons += `<button class="pagination-btn px-4 py-2 text-sm font-medium rounded-full transition duration-150 ${activeClass}" data-page="${i}">${i}</button>`;
        }

        if (endPage < totalPages) {
            if (endPage < totalPages - 1) buttons += `<span class="px-2 py-2 text-sm text-gray-500">...</span>`;
