...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4 animate__animated animate__fadeInDown">
        <span class="text-muted fw-light">Kepegawaian /</span> Data Guru (Pendidik)
    </h4>
    ...
    <div class="card border-0 shadow-soft rounded-4 animate__animated animate__fadeInUp">
        <div class="card-header bg-transparent py-4 border-bottom">
            <h5 class="mb-0 fw-bold text-dark">Daftar Pendidik Wilayah KCD</h5>
        </div>
        ...
        <div class="card-body mt-4 pb-2">
            <form action="{{ route('admin.gtk.guru.index') }}" method="GET" id="filterForm">
                ...
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-2">
                        <a href="{{ route('admin.gtk.guru.index') }}" class="btn btn-outline-danger w-100 rounded-pill fw-bold shadow-xs">
                    ...
                    <div class="col-12 mt-3">
                        <div class="filter-search-group d-flex align-items-center pe-1">
                            ...
                            <input type="text" id="searchInput" name="search" class="form-control px-3 py-2" placeholder="Cari Nama Guru, NIP, atau NUPTK..." value="{{ request('search') }}" autocomplete="off">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light">
                        <tr>
                            ...
                            <th class="py-3 text-uppercase text-muted" style="font-size: 0.75rem; font-weight: 700;">Status Kepegawaian</th>
                            <th class="py-3 text-uppercase text-muted" style="font-size: 0.75rem; font-weight: 700;">Jabatan</th>
                            <th width="1%" class="py-3 text-uppercase text-muted text-end pe-4" style="font-size: 0.75rem; font-weight: 700;">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="border-top-0">
                        @forelse ($gurus as $index => $gtk)
                        <tr>
                            <td class="ps-4 fw-medium text-muted">{{ $gurus->firstItem() + $index }}</td>
                            <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="d-flex flex-column text-truncate" style="max-width: 250px;">
                                        ...
                                        <small class="text-muted font-monospace" style="font-size: 0.75rem;">
                                            @if($gtk->nip && trim($gtk->nip) != '-') <span class="text-primary">NIP. {{ $gtk->nip }}</span>
                                            @elseif($gtk->nuptk && trim($gtk->nuptk) != '-') <span class="text-info">NUPTK. {{ $gtk->nuptk }}</span>
                                            @else <span class="opacity-50">Belum ada Identitas</span> @endif
                                        </small>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <span class="badge bg-label-secondary rounded-pill px-3 py-2 fw-bold text-truncate" style="max-width: 200px;">
                                    <i class="bx bx-buildings me-1"></i> {{ $gtk->sekolah->nama ?? 'Belum Terhubung' }}
                                </span>
                            </td>
                            <td class="text-center">
                                <span class="fw-bold {{ $gtk->jenis_kelamin == 'L' ? 'text-info' : 'text-danger' }}">
                            ...
                            <td><span class="text-dark small fw-medium">{{ $gtk->jabatan_ptk_id_str ?? '-' }}</span></td>
                            <td class="text-end pe-4">
                                <a href="{{ route('admin.gtk.show', $gtk->id) }}" class="btn btn-sm btn-icon btn-label-primary rounded-circle shadow-none" data-bs-toggle="tooltip" title="Lihat Profil Guru">
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-5">
                                <div class="d-flex flex-column align-items-center my-3">
                                    ...
                                    <h6 class="fw-bold text-dark mb-1">Data Guru Tidak Ditemukan</h6>
            ...
            <div class="card-footer border-top bg-transparent py-3 px-4">
                <div class="row align-items-center g-3">
                    <div class="col-md-6 d-flex align-items-center flex-wrap gap-2">
                        ...
                        <form action="{{ route('admin.gtk.guru.index') }}" method="GET" class="d-inline-block m-0">
                        ...
                        <span class="text-muted fw-semibold small ms-2 border-start ps-3">Total: <span class="text-dark">{{ $gurus->total() }}</span> Guru</span>
                    </div>
