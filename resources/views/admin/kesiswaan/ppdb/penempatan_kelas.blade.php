                ...
                <div class="row mb-3">
                        ...
                        <form method="GET" action="{{ route('admin.kesiswaan.ppdb.penempatan-kelas.index') }}">
                            ...
                            <select name="jurusan" class="form-select" onchange="this.form.submit()">
                                ...
                                @foreach($jurusans as $jurusan)
                                    <option value="{{ $jurusan->kompetensi }}" {{ request('jurusan') == $jurusan ? 'selected' : '' }}>
                                        {{ $jurusan->kompetensi }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="pilih_kelas" class="form-select">
                            ...
                            @foreach ($kelas as $r)
                                @php
                                    $romawiMap = [1 => 'I', 7 => 'VII', 10 => 'X'];
                                    $tingkat = $romawiMap[$r->tingkat] ?? $r->tingkat;

                                    if ($r->tingkat == 10 && $r->kompetensiPpdb) {
                                        // Contoh: X PPLG 1
                                        $nama = "{$tingkat} {$r->kompetensiPpdb->kode} {$r->rombel}";
                                    } else {
                                        // Contoh: VII A
                                        $nama = "Kelas {$tingkat} {$r->rombel}";
                                    }
                                @endphp
